...
ampicustays <- amputeData(icustays_tble, perc = 0.05)


amp_results <- miceRanger(ampicustays,
                          m = 3,
                          valueSelector = "meanMatch", 
                          verbose=FALSE,
                          max.depth=10)

saveRDS(as.data.frame(amp_results),
        file = "amp_results.rds")
```

```{r, eval=F}

colSums(is.na(ampicustays))
sum(is.na(ampicustays))
