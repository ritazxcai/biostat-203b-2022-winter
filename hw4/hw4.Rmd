...
```
```{r, eval=F}
saveRDS(icu_imp, file = "amp_results.rds")
```
...
plotVars <- c("thirty_day_mort")

plotList <- lapply(
    plotVars
  , function(x) {
    missIndx <- is.na(icu_cohort1[,get(x)])
    impVsAmp <- data.table(
      originalData = icu_cohort1[missIndx,x]
      , imputedData = data1[missIndx,get(x)]
    )
    return(
