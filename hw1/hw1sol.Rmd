...
Here is the link to my PhysioNet credential:
...
**Solution:**
A total of 66155 rows are in `admissions.csv.gz`. The bash script below displays the number of rows in `admissions.csv.gz`:
```{bash}
...
**Solution:**
The following bash script counts the number of unique subject IDs. **Rationale:** From last part we know that each line contains fields separated by `,` and that the subject IDs are displayed in the first column. Thus, we use `awk` command to search and print only the first column. Next, we use the `uniq -u` command to display the unique lines in the sorted list of subject IDs. Finally, we use `wc` command to count the number of lines in the list, which corresponds to the number of unique subject IDs in this file.
```{bash}
awk -F, '{ print $1 }' /mnt/mimiciv/1.0/core/admissions.csv.gz | sort | uniq -u | wc -l
...
**Solution:** For `admission_type`, the count of unique values:
```{bash}
awk -F, '{ print $6 }' /mnt/mimiciv/1.0/core/admissions.csv.gz | sort | uniq -u | wc -l
...
```{bash}
awk -F, '{ print $7 }' /mnt/mimiciv/1.0/core/admissions.csv.gz | sort | uniq -u | wc -l
...
```{bash}
awk -F, '{ print $9 }' /mnt/mimiciv/1.0/core/admissions.csv.gz | sort | uniq -u | wc -l
...
```{bash}
awk -F, '{ print $12 }' /mnt/mimiciv/1.0/core/admissions.csv.gz | sort | uniq -u | wc -l
...
```{bash}
grep -o -i 'Elizabeth' pg42671.txt | wc -l
grep -o -i 'Jane' pg42671.txt | wc -l
grep -o -i 'Lydia' pg42671.txt | wc -l
grep -o -i 'Darcy' pg42671.txt | wc -l


```


**Solution:** `-nc` option stands for "no clobber", which means that the local file is not overwritten upon repeated download. In other words, when the option `-nc` is specified, the file from the url would not be retrieved from the world wide web; instead, it would be loaded from the local disk.
...
**Note:** To reproduce the result, change the path-name to locate `middle.sh`.
