...
The `zless` and `zmore` commands are used to paginate the compressed file.
The `zgrep` command is used to search inside the compressed file. The file would be uncompressed if needed and fed to the `grep` command. This command is especially useful for finding and displaying lines with specific features.
