...
```{r, eval=F}
# Merging tibbles & adding variable `thirty_day_mort` #
mimic_icu_cohort <- left_join(icustays_tble, admissions_tble,
			by = c("subject_id")) %>%
	left_join(mimic_icu_cohort, patients_tble,
		by = c("subject_id")) %>%
	left_join(mimic_icu_cohort, labevents_tble,
		by = c("subject_id")) %>%
	left_join(mimic_icu_cohort, chartevents_tble,
		by = c("subject_id")) %>%
	mutate(dth_days = as.duration(deathtime - admittime)) %>%
	mutate(thirty_day_mort = (dth_days/86400 <= 30) * 1) %>%
	select(-dth_days) %>%
	print(width = Inf)
```
