...
**Solution:**
```{bash}
awk -F, '{ print $1 }' /mnt/mimiciv/1.0/icu/icustays.csv.gz | sort -u | wc -l

```
There are a total of 11229 unique subject IDs in the `icustays.csv.gz` file.

```{bash}
awk -F, '{ print $1 }' /mnt/mimiciv/1.0/icu/icustays.csv.gz | wc -l

```
There can be multiple ICU stays for the same subject ID. If unique subject IDs are not sorted out, the total number of IDs are greater, meaning that multiple stays were recorded for some individuals.
