...
dlabitems_tble <- read_csv(str_c(mimic_path, "/hosp/d_labitems.csv.gz"),
      col_select = c(itemid),
      col_types = cols_only(itemid = col_double())) %>%
filter(itemid == 50912 |
       itemid == 50971 |
       itemid == 50983 |
       itemid == 50902 |
...
```{r, eval=F}
labevents_tble %>%
  arrange(labevents_tble, desc(subject_id)) %>%
  group_by(subject_id) %>%
  filter(rank(charttime) < 2) %>%
  print(width = Inf)
...
**Note:** This takes very long to run.
```{r, eval=F}
chartevents_tble <- read_csv(str_c(mimic_path, "/icu/chartevents.csv.gz"),
      col_select = c(subject_id, itemid, charttime, valuenum),
      col_types = cols_only(subject_id = col_double(), 
                            itemid = col_double(), 
                            charttime = col_datetime(), 
...
```{r, eval=F}
chartevents_tble %>%
  arrange(chartevents_tble, desc(subject_id)) %>%
  group_by(subject_id) %>%
  filter(rank(charttime) < 2) %>%
  print(width = Inf)
